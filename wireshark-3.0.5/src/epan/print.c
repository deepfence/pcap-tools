...
/* Hack to keep track of src & dest IP & ports */
char netflow[4*ITEM_LABEL_LENGTH + 500]; /* 4 items + 4 delimiters */
#define DF_RECORD_DELIMITER " ##DF_DELIM## "
...
proto_tree_print_node(proto_node *node, gpointer data)
{
    ...
    gchar Df_Label[ITEM_LABEL_LENGTH], Df_IpAddr[ITEM_LABEL_LENGTH];
    int Df_PortNumber;
    ...
    if (fi->rep) {
        ...
        if (fi->hfinfo->type == FT_IPv4) {
            if (!strncmp(fi->hfinfo->name, "Source", strlen("Source")-1) )
                {
                    /*printf("%s, ", label_str);*/
                    sscanf(label_str, "%*s %s", Df_IpAddr);
                    strcat(netflow, Df_IpAddr);
                    strcat (netflow, DF_RECORD_DELIMITER);
                }
            else if (!strncmp(fi->hfinfo->name, "Destination", strlen("Destination")-1))
                {
                    /*printf("%s, ", label_str);*/
                    sscanf(label_str, "%*s %s", Df_IpAddr);
                    strcat(netflow, Df_IpAddr);
        ...
        if (!strncmp(fi->hfinfo->name, "Source Port", strlen("Source Port")-1) )
            {   /*printf("%s ,", label_str);*/ 
                sscanf(label_str, "%*s %*s %d", &Df_PortNumber);
                /*printf("PortNumber: %d\n", Df_PortNumber);*/
                sprintf(Df_Label, "%d", Df_PortNumber);
                strcat(netflow, Df_Label);
                strcat (netflow, DF_RECORD_DELIMITER);
            }
        else if (!strncmp(fi->hfinfo->name, "Destination Port", strlen("Destination Port")-1))
            {   /*printf("%s\n", label_str);*/ 
                sscanf(label_str, "%*s %*s %d", &Df_PortNumber);
                /*printf("PortNumber: %d\n", Df_PortNumber);*/
                sprintf(Df_Label, "%d", Df_PortNumber);
                strcat(netflow, Df_Label);
                //strcat (netflow, DF_RECORD_DELIMITER); //Now print delimiter before printing data
            }
    }
    ...
    if (0) /* We don't need this to genrate alerts */
...
print_hex_data(print_stream_t *stream, epan_dissect_t *edt)
{
    ...
    int           which_source = 2; // 0 means TLS, 1 means uncompressed, 2 means others
    ...
    for (src_le = edt->pi.data_src; src_le != NULL;
        ...
        if (multiple_sources) {
            name = get_data_source_name(src);
            // printf("%s\n", name);
            /* Skip if the source is not "Decrypted TLS" */
            if ( strncmp(name, "Decrypted TLS", strlen("Decrypted TLS")-1) && \
                  strncmp(name, "Uncompressed entity body", strlen("Uncompressed entity body")-1) )
                continue;
            /* 
        ...
        else /* Skip if there is no source or the source is not "Decrypted TLS" */
            continue;    

        //printf("Name: %s, which source: %d %d\n", name, which_source, not_first_iter);
        if ( !strncmp(name, "Decrypted TLS", strlen("Decrypted TLS")-1) ) {
            // print extra newline starting from second occurrence of multiple TLS frames 
            if (not_first_iter == 1) {
                if(which_source == 0)
                    printf(DF_RECORD_DELIMITER"\n");
                else if(which_source == 1)
                    printf("\n");
            }
            else if (not_first_iter == 0)
                not_first_iter = 1;
            
            printf("%s", netflow);
            which_source = 0;
        }
        else if ( !strncmp(name, "Uncompressed entity body", strlen("Uncompressed entity body")-1) ) {
            which_source = 1;
            //printf(DF_RECORD_DELIMITER);
        }
        printf(DF_RECORD_DELIMITER); // print our delimiter before printing TLS or uncompressed data

        wmem_free(NULL, name); /* moved free to here */
    ...
    //printf("SWARUP: printing extra newline here if needed: %d %d %d\n", multiple_sources, not_first_iter, which_source);
    if (multiple_sources && not_first_iter) {
        if(which_source == 0)
            printf(DF_RECORD_DELIMITER"\n");
        else if(which_source == 1)
            printf("\n");
                      ...
                      guint length, packet_char_enc encoding)
{
    ...
    gchar *line;
    line = (gchar *) g_malloc (length * sizeof(gchar) + 1000);
    ...
    while (i < length) {
        /*if ((i & 15) == 0) {*/
        if (i == length + 1) { /* Will never be true */
            /*
             * Start of a new line.
             */
            j = 0;
        ...
        i++;
        /* if (((i & 15) == 0) || (i == length)) { */
        if (i == length) { /* print at the end, not at every line */
            /*
