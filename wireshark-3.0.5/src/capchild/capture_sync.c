...
sync_pipe_start(capture_options *capture_opts, capture_session *cap_session, info_data_t* cap_data, void (*update_cb)(void))
{
    ...
    if (capture_opts->save_file) {
      // Commenting out these 2 lines stops -w flag to dumpcap, but dumpcap creates 1 temp file
...
sync_pipe_input_cb(gint source, gpointer user_data)
{
    ...
    /* we got a valid message block from the child, process it */
    gchar *old_pcap_file_name;
    int status = 0;
	
    switch(indicator) {
    case SP_FILE:
        old_pcap_file_name = g_strdup(cap_session->capture_opts->save_file); 
	    ...
	    g_free(old_pcap_file_name);
            return FALSE;
        }
	status = 1;
	// Delete the temporrary pcap file after processing if it is not NULL or original filename (first time)
	if( old_pcap_file_name != NULL && strcmp(old_pcap_file_name, cap_session->capture_opts->orig_save_file) )
	    status = remove(old_pcap_file_name);
	if( status == -1 )	
	  {
	    printf("DF-Error: Unable to delete the file %s\n", old_pcap_file_name);
	    perror("DF-Error during remove call to delete file");
	  }
	/* else if( status == 0 )
	    printf("%s file deleted successfully\n", old_pcap_file_name);
	else if(status == 1 )
	  printf("Don't delete file due to NULL file name or original filename during first iteration\n");
	*/
	g_free(old_pcap_file_name);
        break;
